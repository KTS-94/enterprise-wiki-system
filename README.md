...
1인 풀스택 개발자로서 아키텍처 설계, 구현, 배포를 담당했습니다. 베이스 프로젝트인 Docmost(AGPL-3.0)가 위키 엔진의 핵심(에디터, 페이지 관리, 협업 프레임워크)을 제공했으며, 이 레포지토리의 모든 코드는 엔터프라이즈 그룹웨어 통합을 위해 직접 구현한 커스텀 확장입니다.
