...
export async function getPageViewById(
  pageId: string,
): Promise<IPage> {
  const req = await api.post<IPage>("/pages/viewinfo", {
    pageId,
  });
  return req.data;
}
...
export async function verifyPagePassword(
  data: IVerifyPagePassword,
): Promise<IVerifyPagePasswordResponse> {
  const req = await api.post<IVerifyPagePasswordResponse>(
    "/pages/verify-password",
    data,
  );
  return req.data;
}
